...
MIN_ARTICLE_SCORE   = int(os.getenv("MIN_ARTICLE_SCORE", "0"))   # v5: optional quality gate
...
def dedupe(articles: list[dict], threshold: float = 0.72) -> list[dict]:
    ...
    for a in articles:
        ...
        if link and link in seen_links:
            continue
        if any(fuzz.ratio(nt, s) >= threshold * 100 for s in seen_titles):
            continue
        if link:
            seen_links.add(link)
...
# v6: tag articles into company / ai / other categories
def tag_articles(articles: list[dict]) -> list[dict]:
    ...
    company_count = sum(1 for a in articles if a["category"] == "company")
    ai_count      = sum(1 for a in articles if a["category"] == "ai")
    print(f"[{now_str()}] Company:{company_count} | AI:{ai_count} | Total:{len(articles)}")
    return articles
...
def score_article(article: dict) -> int:
    ...
    for w in MID_SIGNAL_WORDS:
        if w in text:
            score += 1
    # v5: company-relevant articles get a bonus
    if article.get("company_tag", "市场动态") != "市场动态":
        score += 5
    return score


def rank_and_select(articles: list[dict], limit: int | None = None) -> list[dict]:
    limit = limit or max(MAX_PUSH_ITEMS * 2, 20)
    for a in articles:
        a["score"] = score_article(a)
    articles.sort(key=lambda x: x["score"], reverse=True)
    selected = articles[:limit]
    print(f"[{now_str()}] After ranking: top {len(selected)} selected (limit={limit})")
    return selected
...
def main():
    ...
    # v6: tag articles into company / ai / other
    articles_new = tag_articles(articles_new)
    ...
    ranked = rank_and_select(articles_new)

    # Optional: quality gate via MIN_ARTICLE_SCORE
    ...
    if not ranked:
        print(f"[{now_str()}] All articles scored below MIN_ARTICLE_SCORE={MIN_ARTICLE_SCORE}, skipping.")
        return

    # Only mark ranked (actually processed) articles as sent
    ...
    # Investment brief: only company-relevant articles
    brief_file = None
    if company_articles:
